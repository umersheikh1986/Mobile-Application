...
const DiceAnimation = () => {
  ...
  const [totalCount, setTotalCount] = useState(2); // Initial total count
  ...
  const rollDice = () => {
    ...
    const SWAP_DISTANCE = 150; // Distance to swap
    const DURATION = 1000; // Faster duration for animation
  ...
  useEffect(() => {
    // Roll dice automatically on load and every 2 seconds
    rollDice();
    const interval = setInterval(rollDice, 2000); // Roll every 2 seconds
...
const styles = StyleSheet.create({
  ...
  row: {
    flexDirection: 'row', // Layout in row
    justifyContent: 'center', // Center dice horizontally
    alignItems: 'center',
  },
  diceWrapper: {
    alignItems: 'center',
    marginHorizontal: 10, // Space between dice
  },
  diceContainer: {
    width: 100,
    height: 100,
  ...
  totalCount: {
    ...
    height: 100,
    textAlign: 'center',
    lineHeight: 100, // Vertically center the count
  },
  countWrapper: {
    justifyContent: 'center',
    alignItems: 'center',
